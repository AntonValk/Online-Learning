...
class KalmanMLPproto2_Multi(torch.nn.Module):
    """
    Fully-connected residual architechture with OGD online learning backbone
    """
    ...
    def __init__(self, size_in: int, size_in_MLP: int, size_out: int, variance: float, activation = F.relu,
        ...
        self.fc_layers2 = [torch.nn.Linear(size_in_MLP, layer_width)]
        self.fc_layers2 += [torch.nn.Linear(layer_width, layer_width) for _ in range(num_layers - 1)]
        self.fc_layers2 += [torch.nn.Linear(layer_width, size_out)]
        self.fc_layers2 = torch.nn.ModuleList(self.fc_layers2)

        self.fc_layers3 = [torch.nn.Linear(size_in_MLP, layer_width)]
        self.fc_layers3 += [torch.nn.Linear(layer_width, layer_width) for _ in range(num_layers)]
        self.fc_layers3 += [torch.nn.Linear(layer_width, size_out)]
        self.fc_layers3 = torch.nn.ModuleList(self.fc_layers3)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for layer in self.fc_layers[:-1]:
            h = self.activation(layer(h))
        for layer in self.fc_layers2[:-1]:
            h2 = self.activation(layer(h2))
        for layer in self.fc_layers3[:-1]:
            h3 = self.activation(layer(h3))
        y_hat_MLP = self.fc_layers[-1](h)
        y_hat_MLP2 = self.fc_layers2[-1](h)
        y_hat_MLP3 = self.fc_layers3[-1](h)
        # y_hat = y_hat_lr + y_hat_MLP + 0.25 * proto_pred
        return y_hat_lr, y_hat_MLP, proto_pred, y_hat_MLP2, y_hat_MLP3
...
class MultiClassKalmanMLPproto2(torch.nn.Module):
    """
    Fully-connected residual architechture with OGD online learning backbone
    """
    ...
    def __init__(self, size_in: int, size_in_MLP: int, size_out: int, variance: float, activation = F.relu,
                 ...
                 num_blocks_stage: int = 2, num_layers_stage: int = 2, layer_width_stage: int = 250,
                 dropout: float = 0.3, norm_inputs: bool = True,
                 embedding_dim: int = 32, embedding_size: int = 5000, embedding_num: int = 1, layer_norm: bool = True,
                 b: float=0.99, s: float = 0.2, use_cuda: bool = False, batch_size: int = 1, n_classes: int = 10):
        ...
        self.activation = activation
        self.variance = variance
        self.theta = [torch.zeros(size_in_MLP + 1, requires_grad=False) for _ in range(n_classes)]
        self.Hessian = [self.variance * torch.eye(size_in_MLP + 1, requires_grad=False) for _ in range(n_classes)]
        self.n_classes = n_classes
                
        assert num_layers > 1, f"MLP has to have at least 2 layers, {num_layers} specified"

        self.fc_layers = [torch.nn.Linear(size_in_MLP, layer_width)]
        self.fc_layers += [torch.nn.ReLU(layer_width, layer_width) for _ in range(num_layers - 2)]
        self.fc_layers += [torch.nn.Linear(layer_width, size_out)]
        self.fc_layers = torch.nn.ModuleList(self.fc_layers)
        self.norm_inputs = norm_inputs
    ...
    def online_logistic_regression_step(self, x: torch.Tensor, Y: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            ...
            for i in range(self.n_classes):
                ...
                # y_hat_lr = np.array([1-sigmoid(x @ self.theta[i]).detach(), sigmoid(x @ self.theta[i]).detach()])
                # y_hat_lr = y_hat_lr.T
                # y_hat_lr = torch.Tensor(y_hat_lr)
                # y_hat_lr = y_hat_lr.reshape(1,-1)
                y_hat_lr = sigmoid(x @ self.theta[i]).detach()
                ...
                def rirls(X, y, theta0, Lambda0, theta):
                    ...
                    S_k = H_k @ P_k_old @ H_k.T + (sigmoid(H_k @ theta0).detach().item() * (1-sigmoid(H_k @ theta0).detach().item())) ** 2
                    K_k = P_k_old @ H_k.T * 1/S_k
                    # print("p_k", P_k_old.shape, "H_k", H_k.shape, "s_k", S_k, "theta0", theta0.shape, "Lambda0", Lambda0.shape, "K_k", K_k.shape)
                    theta = theta0 + K_k * (y - sigmoid(H_k @ theta0))
                    Hessian = P_k_old - torch.outer(K_k, K_k) * S_k
                    return theta, Hessian
...
class BackPropOnly(torch.nn.Module):
    """
    Fully-connected residual architechture with OGD online learning backbone
    """
    ...
    def __init__(self, size_in: int, size_in_MLP: int, size_out: int, variance: float, activation = F.relu,
        ...
        self.lr_fc = torch.nn.Linear(size_in_MLP, size_out)
    ...
    def forward(self, x: t.Tensor, *args):
        ...
        for i in range(1, self.max_num_hidden_layers):
            ...
            if i == self.aux_layer - 1:
                ...
                # print(hidden_connections[i - 1].shape)
                # exit()
                linear_x.append(
